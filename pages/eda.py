...
# Sidebar filters
movement = st.sidebar.radio("Type beweging", ["Alle", "Aankomsten", "Vertrekken"])
if movement == "Aankomsten": df = df[df["LSV"] == "L"]
elif movement == "Vertrekken": df = df[df["LSV"] == "S"]
...
with tcol1:
  ...
  fig_h = go.Figure()
  fig_h.add_trace(go.Bar(x=hourly["hour"], y=hourly["vluchten"], name="Vluchten", yaxis="y", opacity=0.3))
  fig_h.add_trace(go.Scatter(x=hourly["hour"], y=hourly["vertr_perc"], name="Vertraagd %", yaxis="y2", mode="lines+markers", line=dict(color="red", width=2)))
  fig_h.update_layout(
      yaxis=dict(title="Vluchten"),
      yaxis2=dict(title="Vertraagd %", overlaying="y", side="right", range=[0, hourly["vertr_perc"].max() * 1.2]),
...
with tcol2:
  ...
  day_labels = {0: "Ma", 1: "Di", 2: "Wo", 3: "Do", 4: "Vr", 5: "Za", 6: "Zo"}
  daily = (
  ...
  fig_d = go.Figure()
  fig_d.add_trace(go.Bar(x=daily["dag"], y=daily["vluchten"], name="Vluchten", yaxis="y", opacity=0.3))
  fig_d.add_trace(go.Scatter(x=daily["dag"], y=daily["vertr_perc"], name="Vertraagd %", yaxis="y2", mode="lines+markers", line=dict(color="red", width=2)))
  fig_d.update_layout(
      yaxis=dict(title="Vluchten"),
      yaxis2=dict(title="Vertraagd %", overlaying="y", side="right", range=[0, daily["vertr_perc"].max() * 1.2]),
...
fig_rwc = px.bar(
    rwc_stats, x="vertr_perc", y="RWC", orientation="h",
    color="vertr_perc", color_continuous_scale="RdYlGn_r",
    hover_data=["vluchten"],
    labels={"vertr_perc": "Vertraagd %", "RWC": "Baanconfiguratie", "vluchten": "Vluchten"},
)
...
lsv_stats = (
    df.groupby("LSV")
    .agg(vertr_perc=("is_delayed", "mean"), gem_vertraging=("delay_minutes", "mean"), vluchten=("FLT", "size"))
    .reset_index()
)
...
with lcol1:
  ...
  fig_lsv1 = px.bar(
      lsv_stats, x="type", y="vertr_perc", color="type",
      color_discrete_map=colors,
      text_auto=".1%",
      labels={"vertr_perc": "Vertraagd %", "type": ""},
  )
  fig_lsv1.update_layout(height=350, showlegend=False, margin=dict(t=10, b=10))
...
with lcol2:
  ...
  fig_lsv2 = px.bar(
      lsv_stats, x="type", y="gem_vertraging", color="type",
      color_discrete_map=colors,
      text_auto=".1f",
      labels={"gem_vertraging": "Gem. vertraging (min)", "type": ""},
  )
  fig_lsv2.update_layout(height=350, showlegend=False, margin=dict(t=10, b=10))
...
for wcol, (key, label) in zip([wcol1, wcol2], list(weather_labels.items())[:2]):
    with wcol:
        ...
        w_agg[key] = w_agg[key].astype(str)
        fig_w = px.bar(w_agg, x=key, y="vertr_perc", hover_data=["vluchten"],
